...
const SingleBlogg = () => {
 ...
 // Helper function to safely render HTML content and convert class to className
 const renderContentWithHtml = (content) => {
  const imageRegex = /(https?:\/\/.*\.(?:png|jpg|jpeg|gif))/i;
  const youtubeRegex =
   /(https?:\/\/www\.youtube\.com\/watch\?v=([a-zA-Z0-9_-]+))/i;

  const convertClassToClassName = (htmlString) => {
   return htmlString.replace(/class="/g, 'className="');
  };

  let parsedContent = convertClassToClassName(content);

  // Split the content by spaces and check for image/video URLs
  return parsedContent.split(/\s+/).map((word, index) => {
     ...
     <div className="container mx-auto">
      ...
      <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-md transition-colors duration-200">
       <div className="text-gray-700 dark:text-gray-300 text-xl mb-4 transition-colors duration-200">
        {/* Rendering HTML content with media handling */}
        {renderContentWithHtml(blog.content)}
       </div>
